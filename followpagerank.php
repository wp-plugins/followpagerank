...
class followHighPr {
	...
	static $mode = 1; // 1 = follow equal or higher pr, 2 = follow higher pr, 3 = follow anything
	static $debug = false;
	
	public static function check_link($ret) {
				
		$dbgmsg = '';
		$gpr = new GooglePR();
		$gpr->maxCacheAge = 86400;
		$gpr->useCache = true;	
		$thispr = (int)$gpr->GetPR(get_bloginfo('siteurl'));

		preg_match("/[\"\']{1}(http[^\"\']+)[\"\']{1}/i", $ret, $match);

		if(isset($match[1]) && !empty($match[1])) {
			$pr = (int)$gpr->GetPR($match[1]);
			$url = $match[1];
		} else {
			return '<!-- followHighPr: No URL found. -->'.$ret;
		}

		switch(followHighPr::$mode) {
			case 1:
				$dbgmsg = "<!-- PR of %s is %d. This site's PR is %d. Not higher, not following. -->";
				if($pr>$thispr) {
					$dbgmsg = "<!-- PR of %s is %d. This site's PR is %d. Higher, following. -->";
					$ret = preg_replace("/rel=[\"\']external nofollow[\"\']/i","rel=\"external\"", $ret);
				}
				break;
			case 2:
				$dbgmsg = "<!-- PR of %s is %d. This site's PR is %d. Not equal/higher, not following. -->";
				if($pr>=$thispr) {
					$dbgmsg = "<!-- PR of %s is %d. This site's PR is %d. Equal/higher, following. -->";
					$ret = preg_replace("/rel=[\"\']external nofollow[\"\']/i","rel=\"external\"", $ret);
				}
				break;
			case 3:
				$dbgmsg = "<!-- PR of %s is %d. This site's PR is %d. Following anything as set. -->";
				break;
		}	
		
		if(followHighPr::$debug) {
			return sprintf($dbgmsg, $url, $pr, $thispr).$ret;
		}
...
add_filter('get_comment_author_link', array('FollowHighPr', 'check_link'),-999999999);


/* Emre Odabas' code is public domain */
